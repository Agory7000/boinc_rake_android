...
const char* app_name = "rakesearch";
const char* in_template_file = "rakesearch_in";
const char* out_template_file = "rakesearch_out";
...
// create one new job
//
int make_job(int number_of_job) {
    DB_WORKUNIT wu;
    ...
    wuNameBuilder << "test3_r10_wu" << std::setw(16) << std::setfill('0') << std::to_string(number_of_job);
    inputfile << "r10_wu_batch_3_" << std::setw(16) << std::setfill('0') << std::to_string(number_of_job) << ".txt";

    // Create the input file.
    // Put it at the right place in the download dir hierarchy
    //
    retval = config.download_path(wuNameBuilder.str().c_str(), path);
    if (retval) return retval;
    FILE* f = fopen(path, "w");
    if (!f) return ERR_FOPEN;
    fprintf(f, "This is the input file for job %s", wuNameBuilder.str().c_str());
    fclose(f);
    ...
    wu.clear();
    wu.appid = app.id;
    safe_strcpy(wu.name, wuNameBuilder.str().c_str());
    wu.rsc_fpops_est = 1e12;
    wu.rsc_fpops_bound = 1e14;
    wu.rsc_memory_bound = 1e8;
    wu.rsc_disk_bound = 1e8;
    wu.delay_bound = 172800;
    ...
    strcpy(name, inputfile.str().c_str());
    infiles[0] =  name;
